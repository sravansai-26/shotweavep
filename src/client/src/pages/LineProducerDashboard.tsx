...
// --- NEW COMPONENT: Quote Request Modal (No changes needed here) ---
...
      const response = await fetch(`${API_BASE_URL}/api/lp/breakdown`, {
        method: 'POST',
        body: formData,
      });
      
      if (!response.ok) {
        setMessage(`Analysis failed: Server responded with status ${response.status}.`);
        return;
      }
...
      }
    } catch (error) {
      setMessage('Network error during script analysis. Check console for CORS issues.');
    }
  };
...
    try {
      // FIX: Use API_BASE_URL
      const response = await fetch(`${API_BASE_URL}/api/lp/lvr`);
      
      if (!response.ok) {
        setMessage(`Failed to load LVR data: Server responded with status ${response.status}.`);
        return;
      }
      const data = await response.json();
      
...
      } else {
        setMessage('Failed to load LVR data: ' + data.message);
      }
    } catch (error) {
      setMessage('Network error during LVR data retrieval. Check console for CORS issues.');
    }
  };

  // FIX 3: Load LVR data automatically on dashboard mount
  useEffect(() => {
    loadLVRData();
  }, []);
...
          <Film size={32} className="mr-3" /> Line Producer Console
        </h1>
